...
RegisterNetEvent('lg-rental:client:LicenseCheck', function(data)
    license = data.LicenseType
    if license == "driver" then
        QBCore.Functions.TriggerCallback("lg-rentals:server:getDriverLicenseStatus", function(hasLicense)
            if hasLicense  then
                TriggerEvent('lg-rental:client:openMenu', data)
                MenuType = "vehicle"
            else
    ...
    elseif license == "pilot" then
        QBCore.Functions.TriggerCallback("lg-rentals:server:getPilotLicenseStatus", function(hasLicense)
            if hasLicense  then
                TriggerEvent('lg-rental:client:openMenu', data)
                MenuType = "aircraft"
            else
...
RegisterNetEvent('lg-rental:client:openMenu', function(data)
    menu = data.MenuType

    local vehMenu = {
        [1] = {
            header = "Vehicles Avaible To Rent",
            isMenuHeader = true,
        },
    
        [2] = {
            id = 1,
            header = "Return Rented Vehicle(s)",
            txt = Lang:t("task.return_veh"),
            params = {
                event = "lg-rental:client:return",
            }
        }
    }
    ...
    if menu == "vehicle" then
        for k=1, #Config.Vehicles.land do
            ...
            vehMenu[#vehMenu+1] = {
                ...
                params = {
                    event = "lg-rental:client:spawncar",
                    args = {
...
RegisterNetEvent('lg-rental:client:spawncar', function(data)
    ...
    QBCore.Functions.TriggerCallback("lg-rental:server:CashCheck",function(money)
        if money then
            ...
            TriggerServerEvent('lg-rental:server:rentalpapers', plate, vehicleLabel)
        else
...
RegisterNetEvent('lg-rental:client:return', function()
    if SpawnVehicle then
        local Player = QBCore.Functions.GetPlayerData()
        QBCore.Functions.Notify(Lang:t("task.veh_returned"), 'success')
        TriggerServerEvent('lg-rental:server:removepapers')
